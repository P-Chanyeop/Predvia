...
namespace Gumaedaehang
{
    public partial class SourcingPage : UserControl
    {
        ...
        private List<CheckBox> _productCheckBoxes = new List<CheckBox>();
        private Ellipse? _productNameStatusIndicator;
        private Ellipse? _product2NameStatusIndicator;
        private Ellipse? _product3NameStatusIndicator;
        ...
        public SourcingPage()
        {
            ...
            _product2NameStatusIndicator = this.FindControl<Ellipse>("Product2NameStatusIndicator");
            _product3NameStatusIndicator = this.FindControl<Ellipse>("Product3NameStatusIndicator");
            _taobaoPairingStatusIndicator = this.FindControl<Ellipse>("TaobaoPairingStatusIndicator");
            
            // 동적으로 상품 체크박스들을 찾아서 리스트에 추가
        ...
        // 동적으로 상품 체크박스들을 초기화
        private void InitializeProductCheckBoxes()
        {
            _productCheckBoxes.Clear();
            
            // 현재 존재하는 상품 체크박스들을 찾아서 리스트에 추가
            var productCheckBoxNames = new[] { "Product1CheckBox", "Product2CheckBox", "Product3CheckBox" };
            
            foreach (var checkBoxName in productCheckBoxNames)
            {
                var checkBox = this.FindControl<CheckBox>(checkBoxName);
                if (checkBox != null)
                {
                    _productCheckBoxes.Add(checkBox);
        ...
        // 새로운 상품 체크박스를 동적으로 추가하는 메서드 (나중에 사용)
        public void AddProductCheckBox(CheckBox checkBox)
        {
            if (checkBox != null && !_productCheckBoxes.Contains(checkBox))
            {
                _productCheckBoxes.Add(checkBox);
        ...
        // 상품 체크박스를 제거하는 메서드 (나중에 사용)
        public void RemoveProductCheckBox(CheckBox checkBox)
        {
            if (checkBox != null && _productCheckBoxes.Contains(checkBox))
            {
                // 이벤트 핸들러 제거
                ...
                _productCheckBoxes.Remove(checkBox);
        ...
        private void SelectAllCheckBox_Changed(object? sender, RoutedEventArgs e)
        {
            if (_selectAllCheckBox != null)
            {
                ...
                // 모든 상품 체크박스의 상태를 전체 선택 상태와 동일하게 설정
                foreach (var productCheckBox in _productCheckBoxes)
                {
                    if (productCheckBox != null)
                    {
                        productCheckBox.IsChecked = isChecked;
                    }
                }
            }
        }
        ...
        {
            if (_selectAllCheckBox == null || _productCheckBoxes.Count == 0)
                return;
            
            int checkedCount = 0;
            int totalCount = _productCheckBoxes.Count;
            ...
            if (checkedCount == 0)
            {
                _selectAllCheckBox.IsChecked = false; // 아무것도 선택되지 않음
            }
            else if (checkedCount == totalCount)
            {
                _selectAllCheckBox.IsChecked = true; // 모두 선택됨
            }
            else
            {
                _selectAllCheckBox.IsChecked = null; // 부분 선택 (Indeterminate 상태)
            }
        }
